...
class CemSuiteKafkaResourceIT {
    ...
    void setup() {
        ...
        CemSuiteKafkaResource kafkaResource = new CemSuiteKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/cem-suite-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/cem-suite-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
